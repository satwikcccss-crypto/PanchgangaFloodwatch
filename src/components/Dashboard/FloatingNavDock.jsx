  ...
  return (
    <>
      ...
      <div className="hidden md:flex fixed right-6 top-1/2 -translate-y-1/2 z-[150] flex-col items-end gap-3 pointer-events-none">
        ...
        {showStationSubmenu && (
          <div className="pointer-events-auto bg-slate-900/95 backdrop-blur-md border border-slate-700/50 rounded-2xl p-3 shadow-2xl w-56 flex flex-col gap-1 transition-all mr-2">
            ...
            {SENSORS.map(s => (
              <button
                ...
                className={`flex items-center justify-between px-3 py-1.5 rounded-lg text-left text-[10px] font-black uppercase tracking-wider transition-all ${
                  ...
                  ? 'bg-academic-gold text-slate-900'
        ...
        {/* Main Dock */}
        <div className="pointer-events-auto bg-slate-900/90 backdrop-blur-md border border-slate-700/50 rounded-[2rem] p-5 shadow-2xl flex flex-col gap-4 items-center w-16">
          {/* Dashboard Icon */}
          <button
          ...
          {/* Details (Opens active station zoomed modal) */}
          <button
          ...
          <button
            ...
            className={`p-3 rounded-xl transition-all relative group ${
              showStationSubmenu ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800'
            }`}
          ...
          <span className="w-8 h-[1px] bg-slate-700/50" />

          {/* Refresh data */}
          <button
            onClick={onRefreshData}
            className="p-3 rounded-xl text-slate-400 hover:text-emerald-400 hover:bg-slate-800 transition-all relative group"
      ...
      <div className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 z-[150] w-[92%] bg-slate-900/95 backdrop-blur-md border border-slate-700/50 rounded-full py-2 px-3 shadow-2xl flex items-center justify-around gap-1">
        <button
          onClick={() => onNavigate('dashboard')}
          className={`p-2.5 rounded-full transition-all ${
            currentPage === 'dashboard' ? 'bg-academic-gold text-slate-900 shadow-md' : 'text-slate-400'
          }`}
        >
        ...
        <button
          onClick={() => onNavigate('network')}
          className={`p-2.5 rounded-full transition-all ${
            currentPage === 'network' ? 'bg-academic-gold text-slate-900 shadow-md' : 'text-slate-400'
          }`}
        >
        ...
        <button
          onClick={onOpenDetails}
          className="p-2.5 rounded-full text-slate-400 active:scale-95"
        >
        ...
        <select
          ...
          className="bg-slate-800 border border-slate-750 text-white text-[8px] font-black uppercase rounded-lg px-1.5 py-1 max-w-[90px] outline-none"
        >
          {SENSORS.map(s => (
